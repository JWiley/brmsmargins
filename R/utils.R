...
#'   Defaults to `TRUE`.
#' @return A list with the `Window`, if specified else `NULL`,
...
#' @param ROPE Either left as `NULL`, the default, or a numeric vector of
#'   length 2, specifying the lower and upper thresholds for the
#'   Region of Practical Equivalence (ROPE).
#' @param MID Either left as `NULL`, the default, or a numeric vector of
...
#' Check a [brms::brmsfit-class] Object has Random Effects
#'
#' Internal utility function to check whether a [brms::brmsfit-class]
#' object has any random effects or not.
#'
#' @param object An object to be evaluated.
#' @return `TRUE` if any random effects present.
#'   `FALSE` if no random effects present.
#' @keywords internal
...
#'
#' Internal utility function to take a [brms::brmsfit-class] object
#' and extract the link for a specific brms `dpar`.
#'
#' @param object A [brms::brmsfit-class] model object.
...
#'
#' @param link The link named in a [brms::brmsfit-class] object
...
.links <- function(link,
  ...
  if (isTRUE(backtrans %in% c("linear", "identity"))) {
    ...
    ## the 'effect' argument
    inverselink <- switch(link,
      "identity" = "identity",
      "logit"    = "invlogit",
      "log"      = "exp",
      "sqrt"     = "square",
      "inverse"  = "inverse",
      "notsupported"  # default if no match
    )
