...
#> sd(Intercept)        1.18      0.18     0.85     1.57 1.01     1326     2085
#> sd(x)                0.54      0.27     0.05     1.05 1.02      386      955
#> cor(Intercept,x)    -0.18      0.40    -0.81     0.76 1.00     1778     1373
#> 
#> Regression Coefficients:
#>           Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> Intercept    -2.48      0.19    -2.88    -2.12 1.00     1722     2017
#> x             0.95      0.19     0.59     1.34 1.00     3183     2542
#> 
...
#> Error in `[.data.table`(data, , ..n): column(s) not found: [L_1[1,1], L_1[2,1], L_1[1,2], L_1[2,2]]

knitr::kable(ame1$ContrastSummary, digits = 3)
#> Error: object 'ame1' not found
```
...
#> Error: object 'ame.error' not found
```
...
#> Error: object 'ame.noerror' not found
```
...
#> Error: object 'mc.logit' not found
```
...
#> ~ID (Number of levels: 100) 
#>                  Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> sd(Intercept)        1.18      0.15     0.92     1.52 1.01     1265     1683
#> sd(x)                0.36      0.21     0.02     0.77 1.01      232      955
#> cor(Intercept,x)    -0.05      0.42    -0.77     0.87 1.00     1382     1310
#> 
#> Regression Coefficients:
#>           Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> Intercept    -2.46      0.17    -2.83    -2.14 1.00     1375     1907
#> x             0.97      0.15     0.68     1.27 1.00     2867     2782
#> 
#> Draws were sampled using sample(hmc). For each parameter, Bulk_ESS
#> and Tail_ESS are effective sample size measures, and Rhat is the potential
#> scale reduction factor on split chains (at convergence, Rhat = 1).
...
#> Error: object 'ame1.pois' not found
```
...
#> Error: object 'ame2.pois' not found
```
...
knitr::kable(cbind(
  ...
  glm.pois[, .(
    GLMCoef = sprintf("%0.3f", round(Est, 3)),
    GLMCI = sprintf("[%0.3f, %0.3f]", round(`2.5 %`, 3), round(`97.5 %`, 3)))]))
#> Error: object 'mc.pois' not found
```
...
mnb <- brms::brm(
  ...
  chains = 4L, cores = 4L, backend = "cmdstanr",
  silent = 2, refresh = 0, adapt_delta = 0.99)
#> Warning: 1638 of 4000 (41.0%) transitions ended with a divergence.
#> See https://mc-stan.org/misc/warnings for details.
#> Warning: 999 of 4000 (25.0%) transitions hit the maximum treedepth limit of 10.
#> See https://mc-stan.org/misc/warnings for details.
...
#> Warning: There were 1638 divergent transitions after warmup. Increasing
...
#> ~ID (Number of levels: 100) 
#>                  Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> sd(Intercept)        1.31      0.22     0.96     1.59 1.47        8       29
#> sd(x)                0.42      0.35     0.00     0.85 2.33        5        4
#> cor(Intercept,x)    -0.49      0.47    -1.00     0.68 2.02        5        4
#> 
#> Regression Coefficients:
#>           Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> Intercept    -2.62      0.24    -2.99    -2.22 1.62        7        6
#> x             1.09      0.17     0.75     1.31 1.49        7       83
#> 
#> Further Distributional Parameters:
#>                       Estimate                 Est.Error l-95% CI
#> shape 615520000648203337728.00 1066245201681847287848.00    13.10
#>                        u-95% CI Rhat Bulk_ESS Tail_ESS
#> shape 2462080000000000000000.00 2.48        5       NA
#> 
#> Draws were sampled using sample(hmc). For each parameter, Bulk_ESS
#> and Tail_ESS are effective sample size measures, and Rhat is the potential
#> scale reduction factor on split chains (at convergence, Rhat = 1).
...
#> Error: object 'ame1.nb' not found
```
...
#> Error: object 'ame2.nb' not found
```
...
#> Error: object 'ame.cent' not found
```
...
#> Error: object 'ame.centint' not found
```
