...
dbeta <- withr::with_seed(
  seed = 12345, code = {
    nGroups <- 2L
    nObs <- 200L
    d <- data.table(x = rep(0:1, each = nObs))
    d[, y := rbeta(n = nObs * nGroups, shape1 = 2 + x, shape2 = 5)]
    copy(d)
  })

suppressWarnings(
  bayes.beta <- brms::brm(
    formula = y ~ x, family = "beta",
    data = dbeta, iter = 1000, warmup = 500, seed = 1234,
    chains = 2, backend = backend, save_pars = save_pars(all = TRUE),
    silent = 2, refresh = 0)
)

h <- .001
margins.bayes <- brmsmargins(bayes.beta,
            add = data.table(x = c(0, h)),
            CI = 0.95,
            contrasts = cbind(AME = c(-1 / h, 1 / h)))
ame.bayes <- margins.bayes$ContrastSummary

test_that("brmsmargins runs for a fixed effects beta model", {
...
m.freq <- betareg::betareg(y ~ x, data = dbeta)
ame.freq <- summary(margins::margins(m.freq))

test_that("brmsmargins roughly matches margins on a frequentist fixed effects beta regression model", {
