...
if (isTRUE(require(cmdstanr))) {
  if (isTRUE(is.null(cmdstanr::cmdstan_version(error_on_NA = FALSE)))) {
    skip("only run if cmdstanr and cmdstan is installed")
  } else {
    ...
    test_that(".predict works to integrate out random effects in multilevel poisson models", {
      ...
      expect_true(abs(res.integrate$Summary$M[1] - res.samp$M[1]) < .15)
      expect_true(abs(res.integrate$Summary$M[2] - res.samp$M[2]) < .20)
